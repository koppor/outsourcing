 ...
 * @since 3.2
 */
public class DisjointSet<Element> {
	/**
	...
	private class Node {
		...
		Node(Element element, Node parent, int rank) {
			this.parent = parent;
			this.rank = rank;
	 ...
	 * within the set.
	 */
	private final HashMap<Element,Node> objectsToNodes = new HashMap<Element,Node>();
	 ...
	 */
	public Element findSet(Element o) {
	 ...
	 */
	public void removeSet(Element o) {
		Element set = findSet(o);
		if (set == null)
			return;
		for (Iterator<Element> it = objectsToNodes.keySet().iterator(); it.hasNext();) {
	...
	public void union(Element x, Element y) {
		Element setX = findSet(x);
		Element setY = findSet(y);
		...
		if (nodeX.rank > nodeY.rank) {
			nodeY.parent = objectsToNodes.get(x);
		} else {
			nodeX.parent = objectsToNodes.get(y);
			if (nodeX.rank == nodeY.rank)
				nodeY.rank++;
		}
	}
